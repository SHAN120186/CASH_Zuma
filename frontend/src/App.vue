...
const companyId=ref(null),reportScenario=ref('A');
...
const allNav=[['home','◈','Обзор','view'],['report','▥','Cash Flow','export'],['accounts','▣','Банк и касса','view'],['ledger','⇄','Операции','ledger'],['requests','✓','Заявки','requests-view'],['calendar','▦','Календарь','view'],['budgets','◷','Бюджеты','view'],['import','↥','Импорт','import'],['categories','≡','Справочники','catalog'],['users','♙','Пользователи','users'],['audit','⊞','Журнал','audit'],['profile','⚙','Профиль','']];
...
const showCurrency=computed(()=>!['users','audit','categories','import','profile'].includes(page.value));
const navGroups=[['Работа',['home','report','accounts','ledger','requests','calendar']],['Планирование',['budgets','import']],['Администрирование',['categories','users','audit']]];
...
async function loadHome(){
 try{homeAccounts.value=await api('/api/accounts')}catch{homeAccounts.value=[]}
 try{const q=new URLSearchParams({paginated:'true',page:'1',currency:currency.value,q:'',state:'pending'});homeRequests.value=(await api('/api/requests?'+q)).items.slice(0,4)}catch{homeRequests.value=[]}
}
...
async function bootstrap(){boot.value=await api('/api/bootstrap');user.value=boot.value.user;csrf.value=boot.value.csrf;if(!companyId.value)companyId.value=boot.value.companies?.[0]?.id;month.value=month.value.match(/^\d{4}-\d{2}$/)?month.value:boot.value.today.slice(0,7)}
...
async function go(name){menuOpen.value=false;helpOpen.value=false;page.value=name;location.hash=name;search.value='';dateFrom.value='';dateTo.value='';requestStatus.value='';listPage.value=1;expandedRequest.value=null;receiptsOpen.value=false;rows.value=[];await load()}
async function load(){busy.value=true;error.value='';try{
 ...
 if(page.value==='report'){const requestId=++reportRequestId;const c=currency.value,y=year.value,co=companyId.value,sc=reportScenario.value;if(report.value&&(report.value.currency!==c||report.value.year!==Number(y)||report.value.company_id!==co||report.value.scenario!==sc))report.value=null;const result=await api(`/api/report?currency=${c}&year=${y}&company_id=${co}&scenario=${sc}`);if(requestId===reportRequestId&&page.value==='report'&&currency.value===c&&year.value===y&&companyId.value===co&&reportScenario.value===sc)report.value=result;}
 if(page.value==='categories')rows.value=boot.value.categories;
 if(page.value==='audit')auditRefresh.value++;
 if(page.value==='users'){rows.value=await api('/api/users');policy.value=await api('/api/approval-policy')}
...
function account(a=null){open(a?'Редактировать счёт':'Добавить счёт',[
 field('name','Название'),field('company_id','Компания','select',{options:boot.value.companies.map(x=>[x.id,x.name])}),field('kind','Тип','select',{options:[['bank','Банковский счёт'],['cash','Касса']]}),field('currency','Валюта','select',{options:['UZS','USD','EUR'].map(x=>[x,x])}),field('opening','Начальный остаток','number',{min:0}),field('opening_date','Дата начала учёта','date'),field('allow_overdraft','Разрешён овердрафт','checkbox',{required:false}),...(a?[field('reason','Причина исправления','textarea',{minlength:10})]:[])],a||{name:'',company_id:boot.value.companies.find(x=>x.code==='UZGERMED')?.id,kind:'bank',currency:currency.value,opening:'0',opening_date:boot.value.today,allow_overdraft:false},d=>post(a?`/api/accounts/${a.id}`:'/api/accounts',Object.fromEntries(['name','company_id','kind','currency','opening','opening_date','allow_overdraft',...(a?['reason']:[])].map(k=>[k,k==='opening'?String(d[k]):d[k]]))),'Остаток на начало дня. Изменения сохраняются в журнале. Валюта счёта с историей защищена от изменения.')}
...
<template>
 ...
 <div v-else class="app" :class="{'menu-open':menuOpen}">
  ...
  <aside class="side" aria-label="Основное меню">
   <a class="brand" href="#home" @click.prevent="go('home')"><span class="mark">U</span><span class="bname">UZGERMED</span></a>
  ...
  <div class="workspace">
   <header class="topbar">
    ...
    <nav class="crumb" aria-label="Путь"><span>UZGERMED</span><i>/</i><b>{{title}}</b></nav>
   ...
   <main class="page">
    ...
    <template v-if="page==='import'">
     ...
     <section v-if="!preview" class="drop"><div class="ic"><AppIcon name="import"/></div><b>Загрузите CSV или Excel</b><span class="sub">До 5 МБ и 1000 операций. Названия счетов и статей — как в справочниках.</span><div class="actions" style="justify-content:center;margin-top:16px"><a class="button secondary" href="/api/import/template.csv"><AppIcon name="download"/> Шаблон CSV</a><label class="button"><AppIcon name="file"/> Выбрать файл<input type="file" accept=".csv,.xlsx" @change="upload" :disabled="busy" hidden></label></div></section>
     <section v-else class="card"><div class="card-h"><div><h3>Проверено строк: {{preview.count}}</h3><p class="sub" style="margin-top:2px">{{preview.errors.length?'Есть ошибки — исправьте файл и загрузите его снова':'Ошибок не найдено, можно подтверждать'}}</p></div><span class="pill" :class="preview.errors.length?'bad':'good'">{{preview.errors.length?'Ошибок: '+preview.errors.length:'Готово к импорту'}}</span></div><div class="card-b"><p v-for="(e,i) in preview.errors" :key="i" class="error">Строка {{e.line}}: {{e.error}}</p><div class="table-scroll"><table><thead><tr><th>Дата</th><th>Документ</th><th>Контрагент</th><th class="num">Сумма</th></tr></thead><tbody><tr v-for="(r,i) in preview.rows" :key="i"><td>{{r.date}}</td><td>{{r.reference}}</td><td>{{r.counterparty}}</td><td class="num">{{money(r.amount)}}</td></tr></tbody></table></div><div class="form-actions"><button class="secondary" @click="preview=null">Загрузить другой файл</button><button v-if="preview.can_commit" @click="commit" :disabled="busy">Подтвердить импорт {{preview.rows.length}} операций</button></div><p v-if="!preview.can_commit" class="sub">Операции не записаны.</p></div></section>
     <section class="card future-module"><h3>Финансовая модель</h3><span class="pill gray">Позже</span></section>
    </template>

    <!-- Cash Flow -->
    <CashFlowReport v-if="page==='report'&&report" :report="report" :currency="currency" :year="year" :api="api" :can-plan="has('budget')" :companies="boot.companies" :company-id="companyId" :scenario="reportScenario" @company="v=>{companyId=v;load()}" @scenario="v=>{reportScenario=v;load()}" @year="v=>{year=v;load()}" @refresh="load" />
    ...
    <template v-if="page==='users'">
     ...
     <div class="tb"><h3>Сотрудники</h3><div class="grow"><button @click="editUser()"><AppIcon name="plus"/> Пользователь</button></div></div>
    ...
    <!-- Журнал -->
    <AuditHistory v-if="page==='audit'" :api="api" :refresh="auditRefresh" />
 ...
 <div v-if="documents" class="modal-backdrop"><section class="modal"><div class="section-head"><h2>Документы · {{documents.ledger.reference}}</h2><button class="ghost icon-btn" aria-label="Закрыть" @click="documents=null"><AppIcon name="x"/></button></div><p v-for="d in documents.rows" :key="d.url"><a :href="d.url">{{d.filename}} ↓</a></p><p v-if="!documents.rows.length" class="sub">Документов пока нет.</p><label v-if="has('write')" class="button" style="margin-top:12px">Прикрепить PDF / изображение<input type="file" accept=".pdf,.png,.jpg,.jpeg" @change="attach" :disabled="saving" hidden></label><p class="sub" style="margin-top:12px">До 5 МБ. Документы доступны только авторизованным пользователям с правом просмотра реестра.</p></section></div>
</template>
