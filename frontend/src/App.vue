...
const boot=ref({accounts:[],categories:[],roles:{}}),rows=ref([]),dash=ref(null),report=ref(null),preview=ref(null),search=ref(''),modal=ref(null),form=ref({}),formError=ref(''),saving=ref(false),documents=ref(null);
...
async function load(){busy.value=true;error.value='';try{
 ...
 if(page.value==='report'){const requestId=++reportRequestId;const c=currency.value,y=year.value;if(report.value&&(report.value.currency!==c||report.value.year!==Number(y)))report.value=null;const result=await api(`/api/report?currency=${c}&year=${y}`);if(requestId===reportRequestId&&page.value==='report'&&currency.value===c&&year.value===y)report.value=result;}
 ...
 <div v-else class="app" :class="{'menu-open':menuOpen}">
  ...
  <div class="workspace">
   ...
   <main class="page">
    ...
    <!-- Cash Flow -->
    <CashFlowReport v-if="page==='report'&&report" :report="report" :currency="currency" :year="year" :api="api" :can-plan="has('budget')" @year="v=>{year=v;load()}" @refresh="load" />
