...
const has=p=>p==='requests-view'?['request','view','pay'].some(x=>user.value?.permissions.includes(x)):user.value?.permissions.includes(p), nav=computed(()=>allNav.filter(n=>!n[3]||has(n[3]))), title=computed(()=>allNav.find(n=>n[0]===page.value)?.[2]||'Казначейство');
...
function requestForm(r=null){
 ...
 modal.value.requestDocumentsPlaceholder=true;
}
