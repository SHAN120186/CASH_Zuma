 ...
 <div v-else class="app" :class="{'menu-open':menuOpen}">
  ...
  <div class="workspace">
   ...
   <main class="page">
    ...
    <!-- Cash Flow -->
    <CashFlowReport v-if="page==='report'&&report" :report="report" :currency="currency" :year="year" :api="api" :can-plan="has('budget')" :companies="boot.companies" :company-id="companyId" :scenario="reportScenario" :as-of="reportAsOf" @company="v=>{companyId=v;load()}" @scenario="v=>{reportScenario=v;load()}" @year="v=>{year=v;reportAsOf=`${v}-12-31`;load()}" @as-of="v=>{reportAsOf=v;load()}" @refresh="load" />
